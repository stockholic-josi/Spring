...
zGrid = {
	...
	getGridData : function(urlInfo){
		...
		console.log($.param(info.params,true));
	...
	*/
	setSrolbarWidth : function(){
		...
		if( zGrid.$grid.find("#dataWrap").hasScrollBar() ){
			...
			if(zGrid.initData.autoColumn != true) zGrid.$grid.find("#dataWrap").width(zGrid.columnWidth);
		}
	...
	*/
	setGridData : function(){
		...
		if( zGrid.initData.store.dataList.length == 0 ){
			  ...
			  zGrid.$grid.find("#paging .nav").hide();
		}else{
			$.each(zGrid.initData.store.dataList,function(i){
				zGrid.addRow($(this)[0]);
			});
			
			zGrid.$grid.find("#paging .nav").show();
		}
	...
	*/
	setFixColumn : function(obj){
		...
		if( zGrid.$grid.width()  > zGrid.fixColumnWidth + zGrid.columnWidth)	zGrid.$grid.width(zGrid.fixColumnWidth + zGrid.columnWidth + 20) 
